...
const NewProviders = () => {
  ...
  const addAdditionalFile = () => {
    if (additionalFiles.length < 5) {
      setAdditionalFiles([...additionalFiles, { name: '', label: `Documento ${additionalFiles.length + 3}` }]);
    }
  };
  ...
  const handleAdditionalFileChange = (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
    const newFiles = [...additionalFiles];
    newFiles[index] = {
      ...newFiles[index],
...
/>
        ...
        <Tabs.Content value="step4">
          <Flex direction="column" gap="3">
            <Text size="4" weight="bold">Evaluación</Text>
